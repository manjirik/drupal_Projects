	...
	//----------------------------------------------------------------------------	
		...
		{			
			$recordsArray = array();
				...
				if(count($paramArray)>0)
				{
				$qry = "SELECT s.song_name as song_name,u.dmm_company_name as musician_name,u.email as musician_email FROM `songs` as s,`user` as u WHERE s.user_id=u.id and s.id='".$paramArray["songid"]."';";
				$recordsArray = $this->executeQuery($this->conObj, $qry);
				}				
				return $recordsArray;
		}
