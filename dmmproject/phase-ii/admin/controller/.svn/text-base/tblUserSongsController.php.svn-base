...
class tblUserSongsController extends database
{
		...
		//----------------------------------------------------------------------------			
			...
			{									
				$recordsArray = array();
				$qry = "SELECT u.name,s.user_id,s.song_name,s.song_credit,s.file_path,s.metadata,s.reject_reason,s.admin_brief_review,s.admin_review,s.hits,s.download_count,s.is_download,s.billboard_image,s.random,s.status FROM user as u,songs as s where u.id = s.user_id and s.id='".$song_id."'";
				$recordsArray = $this->executeQuery($qry);
				return $recordsArray;	
			}
		...
		//----------------------------------------------------------------------------	
			public function getSongLifestyle($song_id)
			{
				$array = $recordsArray = array();
				$query = sprintf("SELECT * FROM `songs_lifestyle_relation` WHERE sid = %d", $song_id);
				$recordsArray = $this->executeQuery($query);

				for($i = 0; $i < count($recordsArray); $i++)
					$array[$recordsArray[$i]['lsid']] = $recordsArray[$i]['lsid'];
			...
			public function updateSongRecord()
				{
						...
						/* admin_brief_review field changes added by prashant b */
						$qry = "update songs set
						metadata = '".DBVarConv($_REQUEST["metadata"])."',
						reject_reason = '".DBVarConv($_REQUEST["reject_reason"])."',
						admin_brief_review = '".DBVarConv($_REQUEST["admin_brief_review"])."',
						...
						$del_query = sprintf("DELETE FROM `songs_lifestyle_relation` WHERE sid = %d", $_POST['song_id']);
						$recordsArray = $this->executeQuery($del_query);

						$lifestyle = $_POST['lifestyle'];
						$values = array();
						foreach($lifestyle as $value) {
							$values[] = "({$_POST['song_id']}, {$value})";
						}

						$ins_query = "INSERT INTO `songs_lifestyle_relation` (`sid`, `lsid`) VALUES " . implode(", ", $values);
						$recordsArray = $this->executeQuery($ins_query);
