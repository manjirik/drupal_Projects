			...
			if(count($paramArray)>0)
				{		
					...
					switch ($paramArray['type']) {		
						...
						$nameval=$paramArray["searchname"];
						
						if($frmDateVal=="" && $toDateVal=="")
						{
						$qry = sprintf("SELECT * FROM `user` where name like '%%%s%%'", $paramArray["searchname"]);
						}
						else if($nameval=="")
						{
						...
						else
						{
							...
							$qrySub1.=sprintf(" name like '%%%s%%'", $paramArray["searchname"]);
							$qrySub .= " where DATE_FORMAT(create_date, '%Y-%m-%d') >= '".$frmDate."' and DATE_FORMAT(create_date, '%Y-%m-%d') <= '".$toDate."' and $qrySub1 ";
							$qry = "SELECT * FROM `user` $qrySub order by id asc LIMIT ".$paramArray["offset"].", ".$paramArray["rowsPerPage"].";";
						}
						break;
						...
						case "name":
						$qry = "SELECT * FROM `user` order by name  LIMIT ".$paramArray["offset"].", ".$paramArray["rowsPerPage"].";";						
						break;
						
						case "dmm_company_name":
						$qry = "SELECT * FROM `user` order by dmm_company_name  LIMIT ".$paramArray["offset"].", ".$paramArray["rowsPerPage"].";";						
						break;
			...
			{						
				...
				{			
					...
					switch ($paramArray['type']) {
						...
						case "search":
						$searchname=$paramArray["searchname"];
						//echo $qry="select * from `user` where name like '%$searchname%' order by id DESC";
						// $qry = "SELECT COUNT(*) AS numrows FROM `user` where name='".$searchname."';";
							  $qry = sprintf("SELECT COUNT(*) AS numrows FROM `user` where name like '%%%s%%'", mysql_real_escape_string($paramArray["searchname"]));
						break;
