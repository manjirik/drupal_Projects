...
function reg_zone_upload()
{ 
	var btnUpload=$('#upload_zone');
	new AjaxUpload(btnUpload, {
		action: hostURL + '/upload.php?type=reg_zone',
		name: 'uphoto',
		onSubmit: function(file, ext) {
			 if (! (ext && /^(jpg|png|jpeg|gif)$/.test(ext))){ 
				// extension is not allowed 
				show_validation_message('bill_board_errorImg', 'bill_board_errorTxt', 'Only JPG, PNG or GIF files are allowed.');
				return false;
			}
		},
		onComplete: function(file, response) {
			...
			if(1 == responseOBJ.status) {
				$('#img_zone').attr('src', responseOBJ.path);
				$('#reg_zone_path').val(responseOBJ.path);
			} else if(0 == responseOBJ.status) {
				show_validation_message('bill_board_errorImg', 'bill_board_errorTxt', responseOBJ.error);
			}
		}
	});
}
...
{ 
	...
	new AjaxUpload(btnUpload, {
		action: hostURL + '/upload.php?type=reg_billboard',
		name: 'uphoto',
		onSubmit: function(file, ext) {
			 if (! (ext && /^(jpg|png|jpeg|gif)$/.test(ext))){ 
				// extension is not allowed 
		...
		onComplete: function(file, response) {
			var responseOBJ = jQuery.parseJSON(response);
			if(1 == responseOBJ.status) {
				$('#img_bill_board').attr('src', responseOBJ.path);
				$('#reg_billboard_path').val(responseOBJ.path);
			} else if(0 == responseOBJ.status) {
				show_validation_message('bill_board_errorImg', 'bill_board_errorTxt', responseOBJ.error);
			}
		}
...
//	name: 'uphoto',
	...
	new AjaxUpload(btnUpload, {
		action: hostURL + '/upload.php?type=profile_avatar', 
		name: 'uphoto', 
		onSubmit: function(file, ext) {
			 if (! (ext && /^(jpg|png|jpeg|gif)$/.test(ext))){ 
				/* extension is not allowed */
				show_validation_message('listener_avatar_errorImg', 'listener_avatar_errorTxt', 'Only JPG, PNG or GIF files are allowed.');
				return false;
			}
		},
		onComplete: function(file, response) {
			var responseOBJ = jQuery.parseJSON(response);
			if(1 == responseOBJ.status) {
				$('#img_listener_avatar').attr('src', responseOBJ.path);
				$('#reg_avatar_path').val(responseOBJ.path);
			} else if(0 == responseOBJ.status) {
				show_validation_message('listener_avatar_errorImg', 'listener_avatar_errorTxt', responseOBJ.error);
			}
		}
	});
}
