...
class tblNotifyController extends database
{
	...
	//----------------------------------------------------------------------------		
		...
		public function removeNotify($paramArray)
		{			
			$recordsArray = array();
			if(count($paramArray)>0)
			{			
				$recordsArray = array();
				$query = sprintf("DELETE FROM notify WHERE musician_id = %d AND notify_user_id = %d", $paramArray["musician_id"], $paramArray["notify_user_id"]);
