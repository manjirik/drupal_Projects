<?php include("inc/config.php"); 
	include("inc/chkSession.php"); 
?>
<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		<title><?php echo ADMIN_PAGE_TITLE; ?>Home</title>	
		<link rel="stylesheet" type="text/css" media="screen" href="<?php echo ADMIN_CSS_PATH; ?>cms.css" />
		<link rel="stylesheet" type="text/css" media="screen" href="<?php echo ADMIN_CSS_PATH; ?>sdmenu.css" />
		<script type="text/javascript" src="<?php echo ADMIN_JS_PATH; ?>jquery-1.4.2.min.js"></script>
		<script type="text/javascript" src="<?php echo ADMIN_JS_PATH; ?>functions.js"></script>
		<script type="text/javascript" src="<?php echo ADMIN_JS_PATH; ?>sdmenu.js">
			/***********************************************
			* Slashdot Menu script- By DimX
			* Submitted to Dynamic Drive DHTML code library: http://www.dynamicdrive.com
			* Visit Dynamic Drive at http://www.dynamicdrive.com/ for full source code
			***********************************************/
		</script>
		<script type="text/javascript">
			// <![CDATA[
			var myMenu;
			window.onload = function() {
				myMenu = new SDMenu("my_menu");
				myMenu.init();
				myMenu.collapseAll();
			};
		// ]]>
		</script>

		<!--<script type='text/javascript' src='<?php echo ADMIN_JS_PATH; ?>libs/jquery/jquery-1.5.js'></script>-->
		<!--<script type='text/javascript' src="<?php echo ADMIN_JS_PATH; ?>globalScript.js"></script>-->
        
        <link rel="stylesheet" type="text/css" href="<?php echo ADMIN_CSS_PATH; ?>animate-custom1.css" >
    	<link rel="stylesheet" type="text/css" href="<?php echo ADMIN_CSS_PATH; ?>global1.css" >
        <link rel="stylesheet" type="text/css" href="<?php echo ADMIN_CSS_PATH; ?>styles.css" >

	</head>
<body>
<table border="0" align="center" cellpadding="0" cellspacing="0" id="mainwrapper">
  <tr>
    <td height="1">
		<div class="header"><?php include_once(ADMIN_INC_PATH."adminHeaderInnerNoLinks.php"); ?></div>	
	</td>
  </tr>
  <tr>
    <td align="left" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="1" align="left" valign="top" class="leftMenu"><?php include_once(ADMIN_INC_PATH."adminLeftComponent.php"); ?></td>
        <td align="left" valign="top" class="padding rightContent"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td valign="top" class="titlepage">Home</td>
          </tr>
          <tr>
            <td  align="left" valign="top" class="paddingtop">
			<table width="100%" height="800"><tr><td>
				<div data-role="page" id="container1">
				</div>

			</td></tr></table>
			</td>
            </tr>
          
        </table></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td height="1"><?php include(ADMIN_INC_PATH."adminFooterInner.php"); ?>	</td>
  </tr>
</table>

</body>
</html>
